...
가져와 Arc 스타일의 사이드바, 손으로 섞은 열두 가지 색 "맛(flavor)", 스페이스,
명령 팔레트, 부드러운 애니메이션, 그리고 유난히 공들인 첫 실행 경험으로
감쌌습니다 — 열두 개 언어로. 새 버전이 준비되면 GitHub Releases를 통해 조용히
스스로 업데이트합니다.

<p align="center">
  <img src="../images/screenshots/arc-light.webp" alt="Arc 스타일 사이드바와 장미 맛을 적용한 Lokum" width="49%">
  <img src="../images/screenshots/arc-dark.webp" alt="한밤 맛을 적용한 다크 모드 Lokum" width="49%">
</p>
...
- 🧭 **Arc 스타일 사이드바 _또는_ 클래식 탭** — 주소 표시줄이 안에 들어 있는 전체 높이 사이드바, Firefox의 세로 탭, 또는 클래식 탭 줄. 언제든 바꿀 수 있고 창이 실시간으로 재배치됩니다.
...
| | 맛 | 분위기 |
| --- | --- | --- |
| 🌹 | **장미** (Gül) | 장미수로 만든 클래식 로쿰, 발그레한 분홍빛. *(기본값)* |
| 🌰 | **피스타치오** (Fıstık) | 싱그러운 피스타치오 그린, 차분하고 집중되는 색. |
| 🍋 | **레몬** (Limon) | 밝은 아침을 위한 햇살 같은 레몬 제스트. |
| 🍎 | **석류** (Nar) | 깊은 석류빛 빨강, 대담하고 상큼하게. |
| 🍊 | **오렌지** (Portakal) | 따뜻한 오렌지와 베르가못의 빛. |
| 🌿 | **민트** (Nane) | 봄바람처럼 상쾌한 시원한 민트. |
| 💜 | **라벤더** (Lavanta) | 꿈같은 오후를 위한 부드러운 라벤더. |
| 🤍 | **매스틱** (Sakız) | 크리미한 매스틱 아이보리, 조용하고 우아하게. |
| 🥥 | **코코넛** (Hindistan cevizi) | 코코넛 화이트, 깔끔하고 미니멀하게. |
| ☕ | **터키 커피** (Kahve) | 진한 터키 커피의 갈색. |
| 🌙 | **한밤** (Gece) | 올빼미족을 위한 별빛 가득한 한밤의 자두색. |
| 🎨 | **모둠** (Karışık) | 모든 색을 천천히 흘러가는 모둠 로쿰 한 상자. |
...
| 레이아웃 | 모습 |
| --- | --- |
| **사이드바 (Arc 스타일)** *(기본값)* | 탭, 주소 표시줄, 탐색 버튼이 전체 높이의 사이드바 하나에 모여 있습니다. 페이지는 둥근 카드처럼 맛 위에 떠 있고, 그 위의 얇은 제목 줄에 페이지 제목, 사이트, *링크 복사* 버튼이 표시됩니다. |
...
- **떠 있는 명령 표시줄** — Arc처럼 주소 표시줄이 페이지 위 가운데에 크게 열립니다.
...
<p align="center">
  ...
  <img src="../images/screenshots/welcome-layout.webp" alt="환영 투어: 레이아웃 고르기" width="49%">
</p>
...
3. **레이아웃** — Arc 스타일 사이드바, 세로 탭, 클래식 탭, 사이드바 위치와 컴팩트 모드를 실시간으로 미리 봅니다.
...
| **레이아웃** | Arc 사이드바 / 세로 / 클래식, 사이드바 위치, 컴팩트 모드, 떠 있는 명령 표시줄, 새 탭 위치, 닫기 버튼, 미니 플레이어, 북마크 도구 모음, 도구 모음 사용자 지정. |
...
빌드되며 그 안의 Firefox 브랜딩을 제거합니다. Arc는 The Browser Company의 상표이며
이 회사 역시 Lokum과 관련이 없습니다 — Lokum은 단지 그 아이디어에서 영감을
받았을 뿐입니다. uBlock Origin은 Raymond Hill과 기여자들이 만들었습니다.
