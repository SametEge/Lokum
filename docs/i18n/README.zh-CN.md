...
它保留了你早已信任的引擎、安全性和扩展，再用 Arc 风格的侧边栏、十二种手工调制的
颜色“口味”、空间（Spaces）、命令面板、柔和的动画，以及格外用心的首次设置把它们
包装起来——支持十二种语言。新版本一准备好，它就会通过 GitHub Releases 悄悄地自动
更新。

<p align="center">
  <img src="../images/screenshots/arc-light.webp" alt="使用 Arc 风格侧边栏和玫瑰口味的 Lokum" width="49%">
...
- 🧭 **Arc 风格侧边栏，_或_ 经典标签页** —— 内含地址栏的全高侧边栏、Firefox 的垂直标签页，或经典的标签栏。随时切换，窗口会实时重新排布。
...
| | 口味 | 氛围 |
| --- | --- | --- |
| 🌹 | **玫瑰**（Gül） | 经典玫瑰水软糖，泛着红晕的粉色。*（默认）* |
| 🌰 | **开心果**（Fıstık） | 清新的开心果绿，平静而专注。 |
| 🍋 | **柠檬**（Limon） | 阳光般的柠檬皮，点亮明媚的早晨。 |
| 🍎 | **石榴**（Nar） | 深邃的石榴红，大胆又多汁。 |
| 🍊 | **橙子**（Portakal） | 橙子与佛手柑的温暖光泽。 |
| 🌿 | **薄荷**（Nane） | 清凉薄荷，像春风一样清爽。 |
| 💜 | **薰衣草**（Lavanta） | 柔和的薰衣草，陪你度过梦幻的午后。 |
| 🤍 | **乳香**（Sakız） | 奶油般的乳香象牙白，安静而优雅。 |
| 🥥 | **椰子**（Hindistan cevizi） | 椰子白，干净而简约。 |
| ☕ | **土耳其咖啡**（Kahve） | 浓郁的土耳其咖啡棕。 |
| 🌙 | **午夜**（Gece） | 为夜猫子准备的星空午夜梅紫。 |
| 🎨 | **什锦**（Karışık） | 一盒什锦软糖，缓缓流转过每一种颜色。 |
...
| 布局 | 外观 |
| --- | --- |
| **侧边栏（Arc 风格）** *（默认）* | 标签页、地址栏和导航按钮都在同一个全高侧边栏中。页面像一张圆角卡片悬浮在口味之上，上方的细标题条显示页面标题、网站和 *复制链接* 按钮。 |
...
<p align="center">
  ...
  <img src="../images/screenshots/welcome-layout.webp" alt="欢迎导览：选择布局" width="49%">
</p>

1. **欢迎** —— 悬浮的 3D 软糖方块、飘落的糖粉，以及语言切换。
2. **口味** —— 十二块裹着糖粉的方块落进托盘；点一块，整个浏览器就会在一阵糖粉中变色。选择浅色、深色或自动，或点 **给我惊喜** 来一次口味轮盘。
3. **布局** —— Arc 风格侧边栏、垂直标签页或经典标签页，以及侧边栏位置和紧凑模式，实时预览。
...
| **布局** | Arc 侧边栏 / 垂直 / 经典、侧边栏位置、紧凑模式、悬浮命令栏、新标签页位置、关闭按钮、迷你播放器、书签工具栏、定制工具栏。 |
...
Arc 是 The Browser Company 的商标，该公司同样与 Lokum 无关——Lokum 只是受到了它的
理念启发。uBlock Origin 由 Raymond Hill 及贡献者开发。
